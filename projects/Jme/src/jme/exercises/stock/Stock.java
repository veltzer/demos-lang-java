...
public class Stock extends MIDlet implements CommandListener {
	...
	public Stock() {
		...
		// Initialize text box.
		textField = new TextField("Symbol", null, 10, TextField.ANY); // Allow
																		// up to
																		// 10
																		// characters
																		// per
	...
	public void commandAction(Command c, Displayable d) {
		if (c == submitCmd) {
			...
			if (textField.getString().length() > 0) {
				retrieveQuote(textField.getString()); // @todo Should do this in
														// separate thread,
														// while giving user an
														// indication of
														// progress
			}
		} else if (c == helpCmd) {
	...
	private void retrieveQuote(String symbol) {
		...
		try {
			...
			conn.setRequestMethod(HttpConnection.GET); // Not required, since
														// GET is default. Can
														// also be used for POST
														// request.
			conn.setRequestProperty("User-Agent",
					"Profile/MIDP-1.0 Configuration/CLDC-1.0"); // Example of
																// setting
																// request
																// property
	...
	private void showHelp() {
		...
		helpAlert.setString(
				"A simple stock ticker application. Enter a valid stock symbol in the text field and use the SUBMIT command to retrieve a current quote.");
