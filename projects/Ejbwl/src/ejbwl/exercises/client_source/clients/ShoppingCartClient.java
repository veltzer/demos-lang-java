package ejbwl.exercises.client_source.clients;

/**
 * A simple client for testing ShoppingCart. This client would: <ul> <li> Obtain
 * a stub to a shopping cart <li> Add some book titles to it <li> Place the
 * order (Receiving a confimration id)
 */
public abstract class ShoppingCartClient {
	public static void main(String[] args) {
		// Lookup - here you should lookup and create the ShoppingCart EJB:
		// Add books - here you should add books to your shopping cart
		// (Assuming these books are in the DB and were previously added using
		// BookStoreAdmin EJB).
		// view books in cart - here you should print all the books currently in
		// your cart:
		// place order - here you should place the order and print the
		// confirmation number:
		// release resources - here you should remove the EJB:
	}
}
