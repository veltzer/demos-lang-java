...
abstract class ThreadAffinity {
    ...
    public static void setThreadAffinityMask(final long threadID, final long mask) {
        ...
        try {
            ...
            if (ret < 0) {
                throw new RuntimeException("sched_setaffinity( " + threadID + ", (" + cpuMaskSize + ") , &(" + mask + ") ) return " + ret);
            }
        } catch (Throwable e) {
            throw new RuntimeException(e);
        }
    ...
    public static int[] parseCoresIndexes(final String str,
        ...
        for (int i = 0; stok.hasMoreTokens(); i++) {
            ...
            try {
                ...
                throw new RuntimeException(new ParseException("Can't parse [" + i + "]='" + token + "' as Integer", i));
            }
            if (index > maxIndex || index < 0) {
                throw new RuntimeException(new ParseException("Core index[" + i + "]=" + index + " is out of bounds [0," + maxIndex + "]", i));
            }
            indexes[i] = index;
        }
        return indexes;
