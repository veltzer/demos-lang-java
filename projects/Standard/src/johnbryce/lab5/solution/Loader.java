...
public class Loader {
	...
	private Object instantiate(Element currClass) throws Exception {
		...
		for (int i = 0; i < params.length; i++) {
			...
			if (type.equals("int")) {
				params[i] = Integer
						.parseInt(currParam.getFirstChild().getNodeValue());
				paramsTypes[i] = int.class;
			} else if (type.equals("short")) {
				params[i] = Short.parseShort(
						paramList.item(i).getFirstChild().getNodeValue());
				paramsTypes[i] = short.class;
			} else if (type.equals("byte")) {
				params[i] = Byte.parseByte(
						paramList.item(i).getFirstChild().getNodeValue());
				paramsTypes[i] = byte.class;
			} else if (type.equals("long")) {
				params[i] = Long.parseLong(
						paramList.item(i).getFirstChild().getNodeValue());
				paramsTypes[i] = long.class;
			} else if (type.equals("boolean")) {
				params[i] = Boolean.parseBoolean(
						paramList.item(i).getFirstChild().getNodeValue());
				paramsTypes[i] = boolean.class;
			} else if (type.equals("float")) {
				params[i] = Float.parseFloat(
						paramList.item(i).getFirstChild().getNodeValue());
				...
				params[i] = instantiate((Element) (currParam
