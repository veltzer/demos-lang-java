...
public final class FormApproval {
	...
	private void setComponents(String itemID, int units) {
		...
		// title
		JLabel title = new JLabel(labels.getString("title"),
		...
		JLabel dateLabel = new JLabel(labels.getString("dateMsg"),
				(Integer) labels.getObject("align"));
		center.add(dateLabel);
		center.add(new JLabel(""));
		JLabel date = new JLabel(labels.getString("date"),
				(Integer) labels.getObject("align"));
		JLabel dateVal = new JLabel(
				((DateFormat) labels.getObject("dateVal")).format(new Date()),
				(Integer) labels.getObject("align"));
		center.add(date);
		center.add(dateVal);
		JLabel time = new JLabel(labels.getString("time"),
				(Integer) labels.getObject("align"));
		JLabel timeVal = new JLabel(
				((DateFormat) labels.getObject("timeVal")).format(new Date()),
				(Integer) labels.getObject("align"));
		center.add(time);
		center.add(timeVal);
		// order item details
		JLabel orderLabel = new JLabel(labels.getString("orderMsg"),
				(Integer) labels.getObject("align"));
		center.add(orderLabel);
		center.add(new JLabel(""));
		JLabel item = new JLabel(labels.getString("item"),
				(Integer) labels.getObject("align"));
		JLabel itemVal = new JLabel(itemID,
				(Integer) labels.getObject("align"));
		center.add(item);
		center.add(itemVal);
		JLabel unitsLabel = new JLabel(labels.getString("units"),
				(Integer) labels.getObject("align"));
		JLabel unitsVal = new JLabel(
				((NumberFormat) labels.getObject("unitsVal")).format(units),
				(Integer) labels.getObject("align"));
		center.add(unitsLabel);
		center.add(unitsVal);
		JLabel unitPrice = new JLabel(labels.getString("unitPrice"),
				(Integer) labels.getObject("align"));
		double unitPriceValue = getUnitPrice();
		JLabel unitPriceVal = new JLabel(
				((NumberFormat) labels.getObject("unitPriceVal")).format(
						unitPriceValue),
				(Integer) labels.getObject("align"));
		center.add(unitPrice);
		center.add(unitPriceVal);
		JLabel total = new JLabel(labels.getString("total"),
				(Integer) labels.getObject("align"));
		JLabel totalVal = new JLabel(
				((NumberFormat) labels.getObject("unitPriceVal"))
						.format(unitPriceValue * units),
				(Integer) labels.getObject("align"));
		center.add(total);
		center.add(totalVal);
