...
public abstract class BadUI {
	...
	// this method takes the chosen item and the no' of pieces to generage a
	// complete order view
	private void setComponents(String itemID, int units) {

		// title
		JLabel title = new JLabel("Order Details", SwingConstants.CENTER); // not
																			// locale
																			// sensitive
																			// !
		...
		JLabel dateVal = new JLabel((calendar.get(Calendar.MONTH) + 1) + "."
				+ calendar.get(Calendar.YEAR)); // not locale sensitive !
		center.add(date);
		center.add(dateVal);
		JLabel time = new JLabel("Time"); // not locale sensitive !
		JLabel timeVal = new JLabel(calendar.get(Calendar.HOUR) + ":"
				+ calendar.get(Calendar.MINUTE)); // not locale sensitive !
		center.add(time);
		center.add(timeVal);
		// order item details
		...
		JLabel unitPriceVal = new JLabel(unitPriceValue + ""); // not locale
																// sensitive !
